#name Depth of Field (Seperation Stage)
...
void main() {
	...
	vec3 litColor = texture(litSceneTexture, fragmentTexCoord).rgb;

	float nearCoc = clamp(1.0f - coc, 0.0f, 1.0f);
	outNearColor = vec4(litColor, nearCoc);
	
	float farCoc = clamp(coc, 0.0f, 1.0f);
